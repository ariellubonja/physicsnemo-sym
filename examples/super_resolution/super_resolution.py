...
# -----------------------------------------------------------
#  Fake VTK so the real shared libraries are never loaded
...
# sub-modules referenced by the autogenerated vtk.py
for _name in (
    "vtkRenderingOpenGL2",
    "vtkRenderingCore",
    "vtkFiltersCore",
    ...
    sys.modules[f"vtkmodules.{_name}"] = types.ModuleType(f"vtkmodules.{_name}")

# stub package  vtk.util  and module  vtk.util.numpy_support
vtk_util = types.ModuleType("vtk.util")
vtk_numpy = types.ModuleType("vtk.util.numpy_support")

# provide dummy functions that just return the input (or None)
def _dummy(*args, **kwargs): return _np.array([])
vtk_numpy.numpy_to_vtk = _dummy
vtk_numpy.vtk_to_numpy = lambda x: _np.array([])

vtk_util.numpy_support = vtk_numpy
